...
If we are going to talk Truth, I think we should lay more weight on the first action of creating our axioms from nothing. We should accept the (albeit counterintuitive) possibility that building an understanding of the universe from any initial statement could potentially be misled (though, unavoidably so). Our minds are not totally comprehensive, and we have a limited purview of "the way of things," such that we can explain every pixel of the film, but we will never know the crew behind the camera. With this in mind, I have to conclude I can only speak for what we experience, and only ever with a small drop of confidence in a vat of skepticism. 

In these essays, among other things, I will reference empirical observations, site the writings of others (who are often considerably more wise than I), comment on my intuition, and do my best to draw sound logical arguments. In so doing, I may thereby implicitly request the reader accept support for my postulates. However, **in no way will I ever make a Truth claim.** I believe the thing is too hard to grasp, and it is best that we live in a state of exploration in ontology. In the words of Gilles Deleuze:
...
# Taking Care with Logic

We tend to reason through traditional Boolean logic, where things are either `true` or `false`. Alternatively, we could also build *equally useful* logical frameworks where things can be something more (or other than) these two values[^fn:fuzzy]. I'm led to believe that the human ability to develop at least two mutually exclusive but independently consistent forms of logic is indicative of the possibility there are other "ways to reason" we are not privy to (and may never be). In fact, maybe reason itself is not the only alternative to empiricism when it comes to *seeking* knowledge. Thus, concerning logic, I will take care. Specifically, I will make an effort to underpin logical conclusions with support or critique from intuition and analysis of language when I can.

[^fn:fuzzy]: Take for example, the three-valued [Kleene Logic](https://www.researchgate.net/publication/31129467_Kleene%27s_Logic_Generalized), or even the infinite (countable) valued [Łukasiewicz Logic](https://publik.tuwien.ac.at/files/pub-tm_5481.pdf). These are both proven consistent, and have plenty of practical applications today.